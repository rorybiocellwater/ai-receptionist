...
// ─── In-memory call log (replace with a database for production) ───
let callLog = [];
...
// ─── Vapi webhook ─────────────────────────────────────────────────
// Vapi sends call events here. We listen for "end-of-call-report"
app.post('/webhook/vapi', async (req, res) => {
  ...
  // Extract customer details from structured data if available
  ...
  callLog.unshift(logEntry); // newest first
  if (callLog.length > 100) callLog.pop(); // keep last 100

  // Send email to operator
  try {
    await transporter.sendMail({
      from: `"AI Receptionist" <${process.env.EMAIL_FROM}>`,
...
app.get('/api/config', (req, res) => {
  res.json({
    ...
    configured: !!(process.env.EMAIL_FROM && process.env.EMAIL_APP_PASSWORD && process.env.EMAIL_TO),
  });
});
